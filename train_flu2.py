...
import numpy as np
import torch
from covid_extract.hosp_consts import include_cols_flu
...
label_idx = include_cols_flu.index("cdc_flu_hosp")
...
def prefix_sequences(seq, week_ahead=week_ahead):
    ...
    """
    l = len(seq)
    X, Y = np.zeros((l - week_ahead, l, 1)), np.zeros(l - week_ahead)
    for i in range(l - week_ahead):
        X[i, (l - i - 1) :, :] = seq[: i + 1, label_idx : label_idx + 1]
...
# Build model
feat_enc = GRUEncoder(in_size=1, out_dim=60, bidirectional=True).to(device)
...
# Save predictions
os.makedirs(f"./flu_predictions", exist_ok=True)
with open(f"./flu_predictions/{save_model_name}_predictions.pkl", "wb") as f:
